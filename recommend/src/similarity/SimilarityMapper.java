...
public class SimilarityMapper extends Mapper<LongWritable, Text, Text, Text>{
	...
	protected void setup(Context context)//读入matrix2，在map方法执行前执行，只执行一次
		...
		FSDataInputStream in = fs.open(new Path("/recommend/scoring_output/part-r-00000"));
...
//				}
				for(int j = 0;j < temp2.length;j++) {
					if(temp1[i].split("_")[0].equals(temp2[j].split("_")[0])) {
						result += Integer.parseInt(temp1[i].split("_")[1]) * Integer.parseInt(temp2[j].split("_")[1]);
					}
				}
			}
